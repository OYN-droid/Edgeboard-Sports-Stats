#!/usr/bin/env python3
"""Validate the six human-approved NBA Style v1 production Batch 3 portraits."""
...
def main() -> int:
    ...
    if {item.get("canonicalEntityId") for item in exports} != EXPECTED_IDS or len(exports) != 6:
        errors.append("NBA Batch 3 provenance must contain the six expected unique canonical athlete IDs")
    if not EXPECTED_IDS.issubset(manifest_ids):
        errors.append("NBA Batch 3 contains an athlete outside the canonical NBA showcase manifest")
    if (provenance.get("styleVersion") != STYLE_VERSION
        ...
        errors.append("NBA Batch 3 provenance does not record the approved Style v1 state")
    if not provenance.get("reviewMetadata") or any(value != "approved" for value in provenance["reviewMetadata"].values()):
        errors.append("NBA Batch 3 human-review metadata is incomplete")
    ...
    if not EXPECTED_IDS.issubset(active_nba):
        errors.append("NBA active portrait coverage must continue to include every Batch 3 athlete")
    pending_ids = manifest_ids - active_nba
    if pending_ids & active_nba:
        errors.append("NBA pending and active portrait states overlap")

    print("EdgeBoard NBA Illustration Production Batch 3")
