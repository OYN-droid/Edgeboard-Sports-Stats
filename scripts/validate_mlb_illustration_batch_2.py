#!/usr/bin/env python3
"""Validate the six human-approved MLB Style v1 Batch 2 portraits."""
...
PROVENANCE_PATH = ROOT / "docs/assets/illustration-style/edgeboard-mlb-illustration-batch-2-exports.json"
...
def main() -> int:
    ...
    if export_ids != EXPECTED_IDS or len(exports) != 6:
        errors.append("Batch 2 provenance must contain the six expected unique canonical athlete IDs")
    if not EXPECTED_IDS.issubset(manifest_ids):
        errors.append("Batch 2 contains an athlete outside the canonical MLB showcase manifest")
    if (provenance.get("styleVersion") != STYLE_VERSION
        ...
        errors.append("Batch 2 provenance does not record the approved standard Style v1 review state")
    if not provenance.get("reviewMetadata") or any(value != "approved" for value in provenance["reviewMetadata"].values()):
        errors.append("Batch 2 human-review metadata is incomplete")
    notes = provenance.get("sourceSelectionNotes", {})
    if "non-action" not in notes.get("mlb-bryce-harper", "") or "forward-facing" not in notes.get("mlb-fernando-tatis-jr", ""):
        errors.append("Final Bryce Harper or Fernando Tatis Jr. source selection is not documented")
    ...
    print("EdgeBoard MLB Illustration Production Batch 2")
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("PASS · final source selection, source preservation, PNG integrity, alpha, dimensions, provenance, canonical mappings, and registry activation are valid")
    return 0
