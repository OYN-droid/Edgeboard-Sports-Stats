#!/usr/bin/env python3
"""Validate the six human-approved MLB Style v1 Batch 3 portraits."""
...
PROVENANCE_PATH = ROOT / "docs/assets/illustration-style/edgeboard-mlb-illustration-batch-3-exports.json"
...
def main() -> int:
    ...
    if export_ids != EXPECTED_IDS or len(exports) != 6:
        errors.append("Batch 3 provenance must contain the six expected unique canonical athlete IDs")
    if not EXPECTED_IDS.issubset(manifest_ids):
        errors.append("Batch 3 contains an athlete outside the canonical MLB showcase manifest")
    if (provenance.get("styleVersion") != STYLE_VERSION
        ...
        errors.append("Batch 3 provenance does not record the approved standard Style v1 review state")
    if not provenance.get("reviewMetadata") or any(value != "approved" for value in provenance["reviewMetadata"].values()):
        errors.append("Batch 3 human-review metadata is incomplete")
    ...
    print("EdgeBoard MLB Illustration Production Batch 3")
