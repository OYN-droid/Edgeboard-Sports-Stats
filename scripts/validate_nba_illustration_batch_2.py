#!/usr/bin/env python3
"""Validate the six human-approved NBA Style v1 production Batch 2 portraits."""
...
PROVENANCE_PATH = ROOT / "docs/assets/illustration-style/edgeboard-nba-illustration-batch-2-exports.json"
...
def main() -> int:
    ...
    if {item.get("canonicalEntityId") for item in exports} != EXPECTED_IDS or len(exports) != 6:
        errors.append("NBA Batch 2 provenance must contain the six expected unique canonical athlete IDs")
    if not EXPECTED_IDS.issubset(manifest_ids):
        errors.append("NBA Batch 2 contains an athlete outside the canonical NBA showcase manifest")
    ...
    if not EXPECTED_IDS.issubset(active_nba):
        errors.append("NBA active portrait coverage must continue to include every Batch 2 athlete")
    pending_ids = manifest_ids - active_nba
    if pending_ids & active_nba:
        errors.append("NBA pending and active portrait states overlap")

    print("EdgeBoard NBA Illustration Production Batch 2")
