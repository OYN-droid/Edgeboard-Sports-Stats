...
function itemView(viewModel) {
  ...
  return `<article class="workspace-item-detail" aria-labelledby="workspaceItemTitle">
    ...
    <div class="workspace-card-actions"><button type="button" data-open-saved="${escapeHtml(item.id)}">Restore research</button>${item.saveMode !== "snapshot" ? `<button type="button" data-refresh-saved="${escapeHtml(item.id)}">Refresh sample snapshot</button>` : ""}<button type="button" data-share-saved="${escapeHtml(item.id)}">Share read-only snapshot</button><button type="button" class="danger-button" data-delete-saved="${escapeHtml(item.id)}">Delete</button></div>
  </article>`;
}
