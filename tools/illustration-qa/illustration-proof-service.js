...
import { ILLUSTRATION_PROOF_PRODUCTION_STATES, ILLUSTRATION_PROOF_QA_FIELDS, ILLUSTRATION_PROOF_REVIEW_STATES } from "./illustration-style-proof-batch.js";
...
const deliveryPath = (sourcePath) => clean(sourcePath).replace(/\.png$/i, ".webp");
...
export function validateIllustrationProofBatch(slots, { canonicalEntities = [], registry = ILLUSTRATION_REGISTRY } = {}) {
  ...
  slots.forEach((slot, index) => {
    ...
    const activatedEntryIsValid = slot.productionStatus === "approved" && slot.reviewStatus === "approved"
      ...
      && exactEntries[0].assetPath === deliveryPath(slot.assetPath) && exactEntries[0].status === "active"
...
export function approvedProofRegistryEntry(slot, inspection = {}) {
  ...
  return Object.freeze({ ...slot.registryDraft, assetPath: deliveryPath(slot.assetPath), status: "active", styleRole: "production_proof_exemplar" });
}
